...
- (BOOL)loadSetupState {
  return [[UserManager sharedInstance] isSetupComplete];
}

- (void)saveSetupState {
  // The Setup Wizard is only run on factory clean states for the system owner.
  // Ensure they always get Admin privileges regardless of UI state.
  self.adminAccount = YES;

  [[UserManager sharedInstance] createUser:self.username
                                  password:self.password
                                   isAdmin:self.adminAccount];
  [[UserManager sharedInstance] markSetupComplete];
}

- (NSString *)settingsPath {
  NSString *appSupport = [NSSearchPathForDirectoriesInDomains(
      NSApplicationSupportDirectory, NSUserDomainMask, YES) firstObject];
  return [[appSupport stringByAppendingPathComponent:@"VirtualOS"]
      stringByAppendingPathComponent:@"user_settings.plist"];
}
...
- (void)showWizard {
  ...
  self.wizardWindow = [[NSWindow alloc]
  ...
  NSGradient *gradient =
      [[NSGradient alloc] initWithColorsAndLocations:[NSColor colorWithRed:0.05
                                                                     green:0.05
                                                                      blue:0.15
                                                                     alpha:1.0],
                                                     0.0,
...
- (void)showWelcomeStep {
  ...
  // Animated logo area
  NSView *logoContainer = [[NSView alloc]
      initWithFrame:NSMakeRect((width - 120) / 2, 280, 120, 120)];
  logoContainer.wantsLayer = YES;
  logoContainer.layer.cornerRadius = 60;
  logoContainer.layer.backgroundColor = [[NSColor colorWithWhite:1.0
                                                           alpha:0.1] CGColor];
  ...
  NSTextField *title = [self createLabel:@"Welcome to VirtualOS"
                                fontSize:32
                                    bold:YES];
  ...
  NSTextField *subtitle =
      [self createLabel:@"Your new virtual operating system awaits"
               fontSize:16
                   bold:NO];
  ...
  for (NSUInteger i = 0; i < features.count; i++) {
    NSView *featureBox = [[NSView alloc]
        initWithFrame:NSMakeRect(startX + i * (featureWidth + 20), 120,
                                 featureWidth, 50)];
    featureBox.wantsLayer = YES;
    featureBox.layer.backgroundColor = [[NSColor colorWithWhite:1.0
                                                          alpha:0.08] CGColor];
    featureBox.layer.cornerRadius = 10;
    [self.contentView addSubview:featureBox];

    NSTextField *featureLabel = [self createLabel:features[i]
                                         fontSize:14
                                             bold:NO];
  ...
  NSButton *continueBtn = [self createButton:@"Get Started →"
...
- (void)showUsernameStep {
  ...
  NSTextField *title = [self createLabel:@"Create Your Account"
                                fontSize:28
                                    bold:YES];
  ...
  // Avatar circle
  NSView *avatarCircle =
      [[NSView alloc] initWithFrame:NSMakeRect((width - 80) / 2, 280, 80, 80)];
  ...
  // Input card
  NSView *inputCard = [[NSView alloc]
      initWithFrame:NSMakeRect((width - 400) / 2, 140, 400, 120)];
  inputCard.wantsLayer = YES;
  inputCard.layer.backgroundColor = [[NSColor colorWithWhite:1.0
                                                       alpha:0.08] CGColor];
  inputCard.layer.cornerRadius = 12;
  [self.contentView addSubview:inputCard];

  NSTextField *label = [self createLabel:@"What's your name?"
                                fontSize:14
                                    bold:NO];
  ...
  self.usernameField =
      [[NSTextField alloc] initWithFrame:NSMakeRect(20, 40, 360, 32)];
  ...
  NSButton *continueBtn = [self createButton:@"Continue"
                                      action:@selector(validateUsername:)];
...
- (void)showAccountTypeStep {
  ...
  NSTextField *title = [self createLabel:@"Choose Account Type"
                                fontSize:28
                                    bold:YES];
  title.frame = NSMakeRect(0, 380, width, 40);
  title.alignment = NSTextAlignmentCenter;
  ...
  NSView *standardBox =
      [[NSView alloc] initWithFrame:NSMakeRect(100, 220, 400, 70)];
  standardBox.wantsLayer = YES;
  standardBox.layer.backgroundColor = [[NSColor colorWithWhite:1.0
                                                         alpha:0.1] CGColor];
  standardBox.layer.cornerRadius = 10;
  [self.contentView addSubview:standardBox];

  self.standardRadio =
      [[NSButton alloc] initWithFrame:NSMakeRect(15, 20, 30, 30)];
  ...
  NSTextField *standardTitle = [self createLabel:@"Standard"
                                        fontSize:16
                                            bold:YES];
  ...
  NSTextField *standardDesc =
      [self createLabel:@"Can use apps and change personal settings"
               fontSize:12
                   bold:NO];
  ...
  NSView *adminBox =
      [[NSView alloc] initWithFrame:NSMakeRect(100, 130, 400, 70)];
  adminBox.wantsLayer = YES;
  adminBox.layer.backgroundColor = [[NSColor colorWithWhite:1.0
                                                      alpha:0.1] CGColor];
  ...
  NSTextField *adminTitle = [self createLabel:@"Administrator"
                                     fontSize:16
                                         bold:YES];
  ...
  NSTextField *adminDesc =
      [self createLabel:@"Can install apps, change settings, and manage users"
               fontSize:12
                   bold:NO];
...
- (void)showPasswordStep {
  ...
  NSTextField *title = [self createLabel:@"Create a Password"
                                fontSize:28
                                    bold:YES];
  title.frame = NSMakeRect(0, 380, width, 40);
  title.alignment = NSTextAlignmentCenter;
  ...
  // Lock icon
  NSView *lockCircle =
      [[NSView alloc] initWithFrame:NSMakeRect((width - 70) / 2, 300, 70, 70)];
  lockCircle.wantsLayer = YES;
  lockCircle.layer.cornerRadius = 35;
  lockCircle.layer.backgroundColor = [[NSColor colorWithWhite:1.0
                                                        alpha:0.1] CGColor];
  ...
  // Password input card
  NSView *inputCard = [[NSView alloc]
      initWithFrame:NSMakeRect((width - 400) / 2, 110, 400, 170)];
  inputCard.wantsLayer = YES;
  inputCard.layer.backgroundColor = [[NSColor colorWithWhite:1.0
                                                       alpha:0.08] CGColor];
  inputCard.layer.cornerRadius = 12;
  ...
  self.passwordField =
      [[NSSecureTextField alloc] initWithFrame:NSMakeRect(150, 210, 300, 28)];
  ...
  NSTextField *confirmLabel = [self createLabel:@"Confirm Password:"
                                       fontSize:14
                                           bold:NO];
  ...
  self.confirmPasswordField =
      [[NSSecureTextField alloc] initWithFrame:NSMakeRect(150, 145, 300, 28)];
...
- (void)showCompleteStep {
  NSTextField *title = [self createLabel:@"You're All Set!"
                                fontSize:36
                                    bold:YES];
  ...
  NSString *welcomeText = [NSString
      stringWithFormat:@"Welcome, %@!\nYour %@ account is ready.",
                       self.username,
                       self.adminAccount ? @"administrator" : @"standard"];
  ...
  NSButton *finishBtn = [self createButton:@"Start Using VirtualOS"
...
- (void)addProgressIndicator:(NSInteger)step of:(NSInteger)total {
  ...
  for (NSInteger i = 0; i < total; i++) {
    NSView *dot = [[NSView alloc]
        initWithFrame:NSMakeRect(startX + i * (dotSize + spacing), 440, dotSize,
                                 dotSize)];
    dot.wantsLayer = YES;
    ...
    if (i < step) {
      ...
      dot.layer.backgroundColor = [[NSColor colorWithWhite:0.4
                                                     alpha:1.0] CGColor];
    }
